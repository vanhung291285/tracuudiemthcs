...
class DatabaseService {
  ...
  private mapDbToStudent(row: any): Student {
    ...
    return {
      ...
      behaviorGradeHK1: row.behaviorGradeHK1 || row.behavior_grade_hk1 || extra.behaviorGradeHK1 || extra.behavior_grade_hk1 || "",
      behaviorGradeHK2: row.behaviorGradeHK2 || row.behavior_grade_hk2 || extra.behaviorGradeHK2 || extra.behavior_grade_hk2 || "",
      ...
      skippedPeriods: typeof row.skippedPeriods === "number" ? row.skippedPeriods : (typeof row.skipped_periods === "number" ? row.skipped_periods : (typeof extra.skippedPeriods === "number" ? extra.skippedPeriods : 0)),
  ...
  private mapStudentToDb(student: Student): any {
    ...
    if (this.isSnakeCaseSchema && this.isModernSchema) {
      result = {
        ...
        behavior_grade: student.behaviorGrade || "Tốt",
        ...
        skipped_periods: student.skippedPeriods || 0,
        ...
        if (!error && data) {
          const mapped: SchoolClass[] = data.map((row: any) => ({
            ...
            academicYear: row.academicYear || row.academic_year || (() => {
              if (row.id && String(row.id).startsWith("class_")) {
                const parts = String(row.id).split("_");
                if (parts.length >= 3) {
                  const yearCandidate = parts[2];
                  if (yearCandidate && /^\d{8}$/.test(yearCandidate)) {
                    return `${yearCandidate.substring(0, 4)}-${yearCandidate.substring(4, 8)}`;
                  }
                }
              }
              return localClasses.find(lc => lc.id === row.id)?.academicYear || "2025-2026";
            })(),
